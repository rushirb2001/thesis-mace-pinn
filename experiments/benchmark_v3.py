...
jax.config.update("jax_default_matmul_precision", "bfloat16") # Switch ot bfloat16 for Low Prec and Faster Training.
...
print("Loading dataset:", "grey_scott.mat")
b1, b2, c1, c2, ep1, ep2, usol, vsol, t_data, x_data, y_data = load_grey_scott_mat("grey_scott.mat")
...
def save_trained_params(trained_params, base_filename):
    """
    Saves the trained parameters to a versioned pickle file.
    """
    versioned_filename = auto_version_filename(base_filename)
    with open(versioned_filename, 'wb') as f:
        pickle.dump(trained_params, f)
    print(f"Parameters saved to {versioned_filename}")
...
def create_fourier_embedding(key, input_dim=3, num_features=64, scale=10.0): # Changing temporarily scale from 6.0 to 10.0, Remember prior configuration involved 6.0 in bfloat16
    assert num_features % 2 == 0
    B = random.normal(key, shape=(num_features//2, input_dim)) * scale
    return B

B_u = create_fourier_embedding(random.PRNGKey(0), 3, num_features, scale)
B_v = create_fourier_embedding(random.PRNGKey(0), 3, num_features, scale)
...
def compute_residual(params, inputs, ep1, ep2, b1, c1, b2, c2):
    """
    Compute the residuals for the Gray–Scott PDE using selective second derivative computation.
    ...
    The residuals are defined as:
        r_u = u_t - ep1*(u_xx + u_yy) - b1*(1 - u) + c1*u*v^2
        r_v = v_t - ep2*(v_xx + v_yy) + b2*v - c2*u*v^2
    """
    def single_pt_res(pt):
        # Define scalar-valued functions for u and v at a point.
        def u_fun(x):
        ...
        # First derivatives: grad_u = [u_x, u_y, u_t]
        grad_u = jax.grad(u_fun)(pt)
        grad_v = jax.grad(v_fun)(pt)

        # Compute second derivatives selectively:
        # u_xx: second derivative of u with respect to x.
        u_xx = jax.grad(lambda x: jax.grad(u_fun)(x)[0])(pt)[0]
        # u_yy: second derivative of u with respect to y.
        u_yy = jax.grad(lambda x: jax.grad(u_fun)(x)[1])(pt)[1]

        # Similarly for v:
        v_xx = jax.grad(lambda x: jax.grad(v_fun)(x)[0])(pt)[0]
        v_yy = jax.grad(lambda x: jax.grad(v_fun)(x)[1])(pt)[1]

        # Laplacian: u_xx + u_yy and v_xx + v_yy
        lap_u = u_xx + u_yy
        lap_v = v_xx + v_yy

        # Evaluate the functions to get u and v at the point.
        u_val = u_fun(pt)
        v_val = v_fun(pt)

        # Extract time derivatives (assumed to be the third component).
        u_t = grad_u[2]
        v_t = grad_v[2]

        # Compute the residuals according to the PDE.
        ru = u_t - ep1 * lap_u - b1 * (1.0 - u_val) + c1 * u_val * (v_val ** 2)
        rv = v_t - ep2 * lap_v + b2 * v_val - c2 * u_val * (v_val ** 2)
        return ru, rv

    # Vectorize the residual computation over all collocation points.
...
def sample_collocation_points_2d(num_points, x_data, y_data):
    ...
    t_rand = random.uniform(key, (num_points,), minval=0, maxval=2)
...
def partial_res_loss(params, inp, ep1, ep2, b1, c1, b2, c2):
    ...
    return np.mean(ru**2), jnp.mean(rv**2)

def partial_data_loss(params, inp, tar):
    if inp.shape[0] == 0:
        return 0.0
    u_d, v_d = pinn_predict_2d(params, inp)
    loss_u = jnp.mean((u_d - tar[:,0])**2)
    loss_v = jnp.mean((v_d - tar[:,1])**2)
...
def compute_gradient_norms_u(params, data_dict, ep1, ep2, b1, c1, b2, c2):
    ...
    def data_fn(pp):
        return partial_data_loss(pp, data_dict["sup"][0], data_dict["sup"][1])[0]

    g_ic  = jax.grad(ic_fn)(params)["u"]
    g_res = jax.grad(res_fn)(params)["u"]
    g_dat = jax.grad(data_fn)(params)["u"]
...
def compute_gradient_norms_v(params, data_dict, ep1, ep2, b1, c1, b2, c2):
    ...
    def data_fn(pp):
        return partial_data_loss(pp, data_dict["sup"][0], data_dict["sup"][1])[1]

    g_ic  = jax.grad(ic_fn)(params)["v"]
    g_res = jax.grad(res_fn)(params)["v"]
    g_dat = jax.grad(data_fn)(params)["v"]
...
def loss_fn(params, data_dict, ep1, ep2, b1, c1, b2, c2):
    ...
    L_dat_u, L_dat_v = partial_data_loss(params, data_dict["sup"][0], data_dict["sup"][1])
    
    # Total losses for U and V
    total_loss_u = (
        lambda_ic_u * L_ic_u + 
        lambda_res_u * L_res_u + 
        lambda_data_u * L_dat_u
    )
    
    total_loss_v = (
        lambda_ic_v * L_ic_v + 
        lambda_res_v * L_res_v + 
        lambda_data_v * L_dat_v
    )
...
def create_optimizer(
    ...
    """Warmup + Exponential decay for Gray-Scott PINNs"""
    lr_schedule = optax.schedules.warmup_exponential_decay_schedule(
        ...
        transition_begin=warmup_steps,  # Start decay after warmup
...
def train_step(params, opt_state_u, opt_state_v, data_dict, optim_u, optim_v, ep1, ep2, b1, c1, b2, c2):
    ...
    grad_u = clip_gradients(grad_u, max_norm=0.01)
    grad_v = clip_gradients(grad_v, max_norm=0.01)
    ...
    # total_loss = total_loss_u + total_loss_v
...
def train_pinn_2d(
    ...
    """
    global b1, b2, c1, c2, ep1, ep2, usol, vsol, t_data, x_data, y_data, update_freq, global_epoch_count, num_features
    total_steps = usol.shape[0]

    # Build net
    key = random.PRNGKey(0)
    # init for MLP-u
    _, init_u_params = init_u(key, (-1, num_features))  # input: x,y,t => shape(3)
    key, subkey = random.split(key)
    # init for MLP-v
    _, init_v_params = init_v(subkey, (-1, num_features))
    ...
    for w_idx in range(num_windows):
        ...
        ic_inp, ic_tar = get_all_points_at_time(usol, vsol, x_data, y_data, 0, 0)
        ...
        optim_u = create_optimizer(
            peak_lr=1e-3,        # Base LR from literature
            warmup_steps=int(0.3*epochs_per_window),   # 20% of total steps
            decay_steps=int(0.1*epochs_per_window),    # Decay every 10% steps
            decay_rate=0.95,      # Moderate decay
            optimizer_type='lamb' # Better for multi-scale
        )

        optim_v = create_optimizer(
            peak_lr=1e-3,        # Base LR from literature
            warmup_steps=int(0.3*epochs_per_window),   # 20% of total steps
            decay_steps=int(0.1*epochs_per_window),    # Decay every 10% steps
            decay_rate=0.95,      # Moderate decay
        ...
        for epoch in tqdm(range(epochs_per_window), desc="PINN training (2D)"):
            ...
            colloc_inp = sample_collocation_points_2d(batch_colloc, x_data, y_data)
            ...
            if epoch % 1000 == 0 and epoch != 0:
                ...
                # Calculate total norms for weight updates
                total_norm_u = gnorms_u['ic'] + gnorms_u['res'] + gnorms_u['data']
                total_norm_v = gnorms_v['ic'] + gnorms_v['res'] + gnorms_v['data']
                total_norm = total_norm_u + total_norm_v

                lambda_ic_u = alpha_u * lambda_ic_u + ((1 - alpha_u) * (total_norm / (gnorms_u['ic'] + 1e-8)))
                lambda_res_u = alpha_u * lambda_res_u + ((1 - alpha_u) * (total_norm / (gnorms_u['res'] + 1e-8)))
                lambda_data_u = alpha_u * lambda_data_u + ((1 - alpha_u) * (total_norm / (gnorms_u['data'] + 1e-8)))

                lambda_ic_v = alpha_v * lambda_ic_v + ((1 - alpha_v) * (total_norm / (gnorms_v['ic'] + 1e-8)))
                lambda_res_v = alpha_v * lambda_res_v + ((1 - alpha_v) * (total_norm / (gnorms_v['res'] + 1e-8)))
                lambda_data_v = alpha_v * lambda_data_v + ((1 - alpha_v) * (total_norm / (gnorms_v['data'] + 1e-8)))

                lambda_ic_u = jnp.clip(lambda_ic_u, 1e-02, 5e-01)
                lambda_ic_v = jnp.clip(lambda_ic_v, 1e-02, 5e-01)
                # lambda_res_u = jnp.clip(lambda_res_u, 5e02, 1e03)
                # lambda_res_v = jnp.clip(lambda_res_v, 5e02, 1e03)
                lambda_data_u = jnp.clip(lambda_data_u, 1e-05, 5e-04)
                lambda_data_v = jnp.clip(lambda_data_v, 1e-05, 5e-04)

                l2_u, l2_v = visualize_window_end(params, time_end, w_idx, global_epoch_count)
                ...
                if epoch % 1000  == 0:
                    ...
                    print(
                        ...
                        f"│ {sep}Data               │ {grad_fmt(gnorms_u['data'])} │ {grad_fmt(gnorms_v['data'])} │\n"
                        f"├────────────── Loss Weights ────────────────│\n"
                        f"│       Component      │  U-Net   │  V-Net   │\n"
                        ...
                        f"│ {sep}Data               │ {weight_fmt(lambda_data_u)} │ {weight_fmt(lambda_data_v)} │\n"
                        f"╰────────────────────────────────────────────╯"
                    )
                lambda_res_u *= l2_u
                lambda_res_v *= l2_v
...
def visualize_window_end(
    ...
    # 1) Load .mat data
    global b1, b2, c1, c2, ep1, ep2, usol, vsol, t_data, x_data, y_data
    Nx, Ny = usol.shape[1], usol.shape[2]
    ...
    # 5) Output directory
    out_dir = f"figures_opt_newb_fulld_bigmodel/window_{w_idx}/epoch_{epoch}"
...
if __name__=="__main__":
    trained_params, losses = train_pinn_2d(
        ...
        window_slide=100,
        window_size=100,
        epochs_per_window=20000,
        batch_colloc=int(4096*16),
        init_lr=1e-4
    )
    save_trained_params(trained_params, "trained_params_sliding_optimisation_uv_value_version3_100.pkl")