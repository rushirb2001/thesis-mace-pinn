...
def train_step(params, opt_state_u, opt_state_v, data_dict, optim_u, optim_v, ep1, ep2, b1, c1, b2, c2):
    ...
    grad_u = clip_gradients(grad_u, max_norm=10.0)
    grad_v = clip_gradients(grad_v, max_norm=10.0)
...
def train_pinn_2d(
    ...
    # # Optimizer
    # # optim_u, optim_v = create_optimizers(lr_u=1e-3, lr_v=1e-4)  # Higher LR for V
    ...
    for w_idx in range(num_windows):
        ...
        optim_u = create_optimizer(
            ...
            warmup_steps=4000,   # 20% of total steps
            decay_steps=2000,    # Decay every 10% steps
            decay_rate=0.9,      # Moderate decay
        ...
        for epoch in tqdm(range(epochs_per_window), desc="PINN training (2D)"):
            ...
            if epoch % 500 == 0 and epoch != 0:
                ...
                lambda_ic_u = jnp.clip(lambda_ic_u, 1e-03, 1e-02)
                lambda_ic_v = jnp.clip(lambda_ic_v, 1e-03, 1e-02)
                lambda_res_u = jnp.clip(lambda_res_u, 1e02, 1e04)
                lambda_res_v = jnp.clip(lambda_res_v, 1e02, 1e04)
                lambda_data_u = jnp.clip(lambda_data_u, 1e-06, 1e-04)
                lambda_data_v = jnp.clip(lambda_data_v, 1e-06, 1e-04)
...
if __name__=="__main__":
    ...
    save_trained_params(trained_params, "trained_params_sliding_optimisation_uv_value_100_v4.pkl")