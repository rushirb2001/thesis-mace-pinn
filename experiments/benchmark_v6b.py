    ...
    def __call__(self, x):
        for i, dim in enumerate(self.hidden_dims[1:]):
            x = nn.Dense(dim, 
                        ...
                        kernel_init=nn.initializers.he_normal())(x) # Add Initial Coupling layer for Experiment to test if that increases the Residual performance.
            x = nn.BatchNorm(dim, 
                        name=f'{self.name}_batchnorm_{i}', momentum=0.9, epsilon=1e-5)(x) # Possible to remove BatchNorm for better performance.
...
class GrayScottPINN(nn.Module):
    ...
    def __call__(self, x):
        ...
        x_coup = nn.Dense(self.hidden_dims[0],
                    ...
                    kernel_init=nn.initializers.he_normal())(ff)
        x_coup = nn.BatchNorm(name=f'{self.name}_batchnorm_0', momentum=0.9, epsilon=1e-5)(x_coup)
