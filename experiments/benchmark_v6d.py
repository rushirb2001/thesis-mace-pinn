...
class GrayScottPINN(nn.Module):
    ...
    def __call__(self, x):
        ...
        x_coup = nn.Dense(self.hidden_dims[0],
                    name=f'head_dense_0',
                    kernel_init=nn.initializers.glorot_normal())(ff)
...
def compute_loss(state, params, batch, colloc_points, constants):
    ...
    loss_ic_u = jnp.mean((u_pred[:40000] - targets_ic[:, 0])**2)
    loss_ic_v = jnp.mean((v_pred[:40000] - targets_ic[:, 1])**2)
    ...
    with tqdm(total=config['epochs'], ncols=75) as pbar:
        for epoch in range(config['epochs']):
            ...
            if epoch % config['log_freq'] == 0:
                ...
                total_norm_u = (grad_norms['ic_u'] 
                                + grad_norms['res_u'] 
                                # + grad_norms['data_u']
                                )
                total_norm_v = (grad_norms['ic_v'] 
                                + grad_norms['res_v'] 
                                # + grad_norms['data_v']
                                )
                ...
                # lambda_data_u = jnp.clip(lambda_data_u, 1e1, 1e2)
                # lambda_res_u = jnp.clip(lambda_res_u, 1e1, 1e3)

                # lambda_ic_v = jnp.clip(lambda_ic_v, 1e-3, 1e-2)
                # lambda_data_v = jnp.clip(lambda_data_v, 1e1, 1e2)
                # lambda_res_v = jnp.clip(lambda_res_v, 1e1, 1e3)
 
                loss_val_u = (metrics['res_u'] 
                            + metrics['ic_u'] 
                            # + metrics['data_u']
                            ) 
                loss_val_v = (metrics['res_v'] 
                            + metrics['ic_v'] 
                            # + metrics['data_v']
                            )
