...
def partial_res_loss(params, inp, ep1, ep2, b1, c1, b2, c2, M, tol):
    if inp.shape[0] == 0:
        return 0.0
    ru, rv = compute_residual(params, inp, ep1, ep2, b1, c1, b2, c2)

    ru_l = jnp.mean(ru**2, axis=1)
    rv_l = jnp.mean(rv**2, axis=1)

    # Apply the mask M
    ru = lax.stop_gradient(jnp.exp(tol * (M @ ru_l)))
    rv = lax.stop_gradient(jnp.exp(tol * (M @ rv_l)))
...
def compute_gradient_norms_u(params, data_dict, ep1, ep2, b1, c1, b2, c2, M, tol):
    """Gradient norms for U-Net only"""
    def ic_fn(pp):
        return partial_ic_loss(pp, data_dict["ic"][0], data_dict["ic"][1])[0]
    def res_fn(pp):
        return partial_res_loss(pp, data_dict["colloc"], ep1, ep2, b1, c1, b2, c2, M, tol)[0]
    def data_fn(pp):
...
def compute_gradient_norms_v(params, data_dict, ep1, ep2, b1, c1, b2, c2, M, tol):
    """Gradient norms for V-Net only"""
    def ic_fn(pp):
        return partial_ic_loss(pp, data_dict["ic"][0], data_dict["ic"][1])[1]
    def res_fn(pp):
        return partial_res_loss(pp, data_dict["colloc"], ep1, ep2, b1, c1, b2, c2, M, tol)[1]
    def data_fn(pp):
...
def loss_fn(params, data_dict, ep1, ep2, b1, c1, b2, c2, M, tol):
    global global_epoch_count, update_freq

    # Compute all components
    L_ic_u, L_ic_v = partial_ic_loss(params, data_dict["ic"][0], data_dict["ic"][1])
    L_res_u, L_res_v = partial_res_loss(params, data_dict["colloc"], ep1, ep2, b1, c1, b2, c2, M, tol)
    if global_epoch_count % (1*update_freq) == 0:
        L_dat_u, L_dat_v = partial_data_loss(params, data_dict["sup"][0], data_dict["sup"][1])
    else:
        L_dat_u, L_dat_v = 0.0, 0.0
...
def train_step(params, opt_state_u, opt_state_v, data_dict, optim_u, optim_v, ep1, ep2, b1, c1, b2, c2, M, tol):
    # Separate loss calculations
    total_loss_u, total_loss_v = loss_fn(params, data_dict, ep1, ep2, b1, c1, b2, c2, M, tol)
    
    # U-Net gradients
    grad_u = jax.grad(lambda pu: loss_fn(
        {"u": pu, "v": params["v"]}, data_dict, ep1, ep2, b1, c1, b2, c2, M, tol
    )[0])(params["u"])
    
    # V-Net gradients
    grad_v = jax.grad(lambda pv: loss_fn(
        {"u": params["u"], "v": pv}, data_dict, ep1, ep2, b1, c1, b2, c2, M, tol
    )[1])(params["v"])
...
def train_pinn_2d(
    ...
    """
    global b1, b2, c1, c2, ep1, ep2, usol, vsol, t_data, x_data, y_data, update_freq, global_epoch_count
    ...
    for w_idx in range(num_windows):
        ...
        tol = 1e-6
        M = jnp.triu(jnp.ones((100, 100)), k=1).T
        ...
        for epoch in tqdm(range(epochs_per_window), desc="PINN training (2D)"):
            ...
            params, opt_state_u, opt_state_v, loss_val_u, loss_val_v = train_step(
                                                            ...
                                                            optim_u, optim_v, ep1, ep2, b1, c1, b2, c2, M, tol
                                                        )
            loss_val = loss_val_u + loss_val_v
            all_losses.append(loss_val)

            
            if epoch % (5*update_freq) == 0 and epoch != 0:
                ...
                lambda_ic_u = jnp.clip(lambda_ic_u, 1e-01, 1e2)
                lambda_ic_v = jnp.clip(lambda_ic_v, 1e-01, 1e2)

                if epoch % (10*update_freq) == 0:
...
def visualize_window_end(
    ...
    # 5) Output directory
    out_dir = f"figures_opt_resloss_iterative/window_{w_idx+1}/epoch_{epoch-1}"
