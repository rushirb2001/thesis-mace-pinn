...
def compute_residual(params, inputs, ep1, ep2, b1, c1, b2, c2):
    ...
    def single_pt_res(pt):
        # Define scalar-valued functions at the point.
        def u_fun(x):
        ...
        # Get the function values and time derivatives
        u_val = u_fun(pt)
        v_val = v_fun(pt)
        u_t = grad_u[2]
        v_t = grad_v[2]
...
def gradient_centralization(grads):
    """
    Applies gradient centralization: for each weight matrix with more than one dimension,
    subtract the mean over all but the output dimension.
    """
    def centralize(g):
        if g.ndim > 1:
            # Subtract mean over all dimensions except the last one.
            return g - jnp.mean(g, axis=tuple(range(g.ndim - 1)), keepdims=True)
        else:
            return g
    return jax.tree_util.tree_map(centralize, grads)
...
def train_step(params, opt_state_u, opt_state_v, data_dict, optim_u, optim_v, ep1, ep2, b1, c1, b2, c2):
    ...
    grad_u = gradient_centralization(grad_u)
    grad_v = gradient_centralization(grad_v)
    
    # Clip gradients separately
    grad_u = clip_gradients(grad_u, max_norm=0.1)
