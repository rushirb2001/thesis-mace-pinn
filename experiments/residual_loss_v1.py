...
B_u = create_fourier_embedding(random.PRNGKey(0), 3, 96)
B_v = create_fourier_embedding(random.PRNGKey(0), 3, 96)
...
def train_pinn_2d(
    ...
    # init for MLP-u
    _, init_u_params = init_u(key, (-1, 96))  # input: x,y,t => shape(3)
    key, subkey = random.split(key)
    # init for MLP-v
    _, init_v_params = init_v(subkey, (-1, 96))
    ...
    for w_idx in range(num_windows):
        ...
        for epoch in tqdm(range(epochs_per_window), desc="PINN training (2D)"):
            ...
            if epoch % 500 == 0 and epoch != 0:
                ...
                # total_norm_u = gnorms_u['ic'] + gnorms_u['res']
                # total_norm_v = gnorms_v['ic'] + gnorms_v['res']
