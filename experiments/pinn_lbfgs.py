...
def create_mlp(layer_dims):
    ...
    for dim in layer_dims[:-1]:
        layers.append(stax.Dense(dim))
        layers.append(stax.Tanh)  # Better than Tanh for PDEs
    layers.append(stax.Dense(layer_dims[-1]))
    return stax.serial(*layers)

# Build separate MLPs for u and v
init_u, apply_u = create_mlp([64, 64, 64, 1])
...
def create_optimizers():
    """L-BFGS settings aligned with Grey-Scott dynamics"""
    return (
        optax.lbfgs(
            memory_size=5,  # Matches inhibitor's longer interaction range
        ...
        optax.lbfgs(
            memory_size=10,  # Handles activator's chaotic patterns
            linesearch=optax.scale_by_zoom_linesearch(
                max_linesearch_steps=15,
                tol=1e-7,  # Tighter for sharp gradients
            )
        )
    )

def train_step(params, opt_state_u, opt_state_v, data_dict, optim_u, optim_v, ep1, ep2, b1, c1, b2, c2):
    # For U network: define a function f_u(pu) that returns the U loss.
    f_u = lambda pu: loss_fn({"u": pu, "v": params["v"]}, data_dict, ep1, ep2, b1, c1, b2, c2)[0]
    value_and_grad_u = optax.value_and_grad_from_state(f_u)
    value_u, grad_u = value_and_grad_u(params["u"], state=opt_state_u)
    grad_u = clip_gradients(grad_u, max_norm=10.0)
    updates_u, opt_state_u = optim_u.update(grad_u, opt_state_u, params["u"],
                                            value=value_u, grad=grad_u, value_fn=f_u)
    new_params_u = optax.apply_updates(params["u"], updates_u)
    
    # For V network: define a function f_v(pv) that returns the V loss.
    f_v = lambda pv: loss_fn({"u": params["u"], "v": pv}, data_dict, ep1, ep2, b1, c1, b2, c2)[1]
    value_and_grad_v = optax.value_and_grad_from_state(f_v)
    value_v, grad_v = value_and_grad_v(params["v"], state=opt_state_v)
    grad_v = clip_gradients(grad_v, max_norm=100.0)
    updates_v, opt_state_v = optim_v.update(grad_v, opt_state_v, params["v"],
                                            value=value_v, grad=grad_v, value_fn=f_v)
    new_params_v = optax.apply_updates(params["v"], updates_v)
    
    new_params = {"u": new_params_u, "v": new_params_v}
    total_loss = value_u + value_v
...
def train_pinn_2d(
    ...
    # Optimizer
    optim_u, optim_v = create_optimizers()  # Higher LR for V
...
def visualize_window_end(
    ...
    # 5) Output directory
    out_dir = f"figures_opt/window_{w_idx}/epoch_{epoch}"
