...
def update_weights_with_momentum(lambda_w, grad_norms, beta=0.9):
    """Update loss weights using momentum and normalize separately for u and v."""
    components = ['ic_u', 'data_u', 'res_u', 'ic_v', 'data_v', 'res_v']
    for component in components:
        total_norm = sum(grad_norms[c] for c in components[:3]) if 'u' in component else sum(grad_norms[c] for c in components[3:])
        lambda_w[component] = beta * lambda_w[component] + (1 - beta) * (total_norm / (grad_norms[component] + 1e-8))
    
    # Normalize weights
    sum_u = lambda_w['ic_u'] + lambda_w['data_u'] + lambda_w['res_u']
    sum_v = lambda_w['ic_v'] + lambda_w['data_v'] + lambda_w['res_v']
    for c in ['ic_u', 'data_u', 'res_u']:
        lambda_w[c] = lambda_w[c] / sum_u
    for c in ['ic_v', 'data_v', 'res_v']:
        lambda_w[c] = lambda_w[c] / sum_v
    
    # Clip weights
    for c in components:
        lambda_w[c] = jnp.clip(lambda_w[c], 0.01, 10.0)
    return lambda_w

@jax.jit
def compute_approx_grad_norms(params, batch, colloc_points, constants, state):
    """Approximate gradient norms using a single loss evaluation."""
    def loss_fn(params):
        u_pred, v_pred = state.apply_fn(params, batch['inputs'])
        ru, rv = compute_residuals(state, params, colloc_points, constants)
        ic_loss = jnp.mean((u_pred[:1] - batch['targets'][:1, 0])**2) + jnp.mean((v_pred[:1] - batch['targets'][:1, 1])**2)
        data_loss = jnp.mean((u_pred[1:] - batch['targets'][1:, 0])**2) + jnp.mean((v_pred[1:] - batch['targets'][1:, 1])**2)
        res_loss = jnp.mean(ru**2) + jnp.mean(rv**2)
        return {'ic_u': ic_loss, 'ic_v': ic_loss, 'data_u': data_loss, 'data_v': data_loss, 'res_u': res_loss, 'res_v': res_loss}

    # Compute gradients and approximate norms in one pass
    loss_dict, grad_dict = jax.value_and_grad(loss_fn)(params)
    grad_norms = {}
    for key in ['ic_u', 'data_u', 'res_u', 'ic_v', 'data_v', 'res_v']:
        grad_norms[key] = jnp.sqrt(jax.tree_util.tree_reduce(
            lambda x, y: x + jnp.sum(y**2),
            jax.tree_util.tree_map(lambda x: x, grad_dict[key] if key in grad_dict else grad_dict),
            0.0
        ))
    return grad_norms
...
# ====================================================
def logging_metrics(lambda_w, metrics, grad_norms, loss_val, epoch):
    ...
    print(
            ...
            # f"│   V-Loss: {loss_val['v']:.4e} │  NTK Vnet: {000:.2e} │\n"
            f"├──────────── Res Error - Val ───────────────│\n"
            f"│   U-L2: {metrics['res_u']:.3e}    │   V-L2: {metrics['res_v']:.3e}   │\n"
            f"├──────────── Dat Error - Val ───────────────│\n"
            f"│   U-L2: {metrics['data_u']:.3e}    │   V-L2: {metrics['data_v']:.3e}   │\n"
            f"├──────────── InC Error - Val ───────────────│\n"
            f"│   U-L2: {metrics['ic_u']:.3e}    │   V-L2: {metrics['ic_v']:.3e}   │\n"
            # f"├──────────── Gradient Norms ────────────────│\n"
...
def train(config, data):
    ...
    update_interval = 500  # Fixed, sparse updates
    ...
    with tqdm(total=config['epochs'], ncols=150) as pbar:
        for epoch in range(config['epochs']):
            ...
            # Lightweight moving average of total loss
            loss_moving_avg = alpha * loss_moving_avg + (1 - alpha) * total_loss
            
            # Update weights sparsely
            if epoch % update_interval == 0 and epoch > 0:
                ...
                lambda_w = update_weights_with_momentum(lambda_w, grad_norms, beta=0.9)
                
                # Log and visualize
                if epoch % config['log_freq'] == 0:
                    logging_metrics(lambda_w, metrics, grad_norms, loss_val, epoch)
                    visualize_solution(state, data, epoch, t_start + window_size - 1)
            
            # Regular logging
            elif epoch % config['log_freq'] == 0:
                grad_norms = {}  # Empty dict for logging if not computing
                logging_metrics(lambda_w, metrics, grad_norms, loss_val, epoch)
                visualize_solution(state, data, epoch, t_start + window_size - 1)
