...
# init_u, apply_u = create_mlp([128, 256, 64, 1])
# init_v, apply_v = create_mlp([128, 128, 256, 128, 64, 1])

init_u, apply_u = create_mlp([64, 64, 64, 1])
init_v, apply_v = create_mlp([128, 128, 128, 128, 1])

def create_fourier_embedding(key, input_dim=3, num_features=64, scale=2.0):
    assert num_features % 2 == 0
    B = random.normal(key, shape=(num_features//2, input_dim)) * scale
    return B

B = create_fourier_embedding(random.PRNGKey(0), 3, 32)
...
def pinn_predict_2d(params, inputs):
    ...
    mlp_input = fourier_embed(mlp_input, B)
...
def sample_collocation_points_2d(num_points, x_data, y_data, tmin, tmax):
    ...
    x_rand = random.uniform(key, (num_points,), minval=-1, maxval=1)
    y_rand = random.uniform(key, (num_points,), minval=-1, maxval=1)
    # t_rand = random.uniform(key, (num_points,), minval=0.0, maxval=tmax)
    t_rand = ((random.uniform(key, (100,), minval=tmin, maxval=tmax).astype(jnp.int32) * 2) / 100).astype(jnp.float32)
...
def create_optimizers(lr_u=1e-3, lr_v=1e-4, decay_steps=100, decay_rate=0.75):
    sched_u = optax.exponential_decay(lr_u, 5e2, decay_rate)
    sched_v = optax.exponential_decay(lr_v, 5e3, decay_rate)
...
def train_step(params, opt_state_u, opt_state_v, data_dict, optim_u, optim_v, ep1, ep2, b1, c1, b2, c2):
    ...
    grad_u_clipped = clip_gradients(grad_u, max_norm=100.0)
    grad_v_clipped = clip_gradients(grad_v, max_norm=1.0)
...
def train_pinn_2d(
    ...
    # init for MLP-u
    _, init_u_params = init_u(key, (-1, 32))  # input: x,y,t => shape(3)
    key, subkey = random.split(key)
    # init for MLP-v
    _, init_v_params = init_v(subkey, (-1, 32))
...
def visualize_window_end(
    ...
    # 5) Output directory
    out_dir = f"figures_opt_nw/window_{w_idx}/epoch_{epoch}"
