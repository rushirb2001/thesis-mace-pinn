...
init_v, apply_v = create_mlp([128, 128, 128, 1])
...
B_u = create_fourier_embedding(random.PRNGKey(0), 3, 48)
...
def train_pinn_2d(
    ...
    # init for MLP-u
    _, init_u_params = init_u(key, (-1, 48))  # input: x,y,t => shape(3)
    ...
    for w_idx in range(num_windows):
        ...
        for epoch in tqdm(range(epochs_per_window), desc="PINN training (2D)"):
            ...
            if epoch % 500 == 0 and epoch != 0:
                ...
                lambda_ic_u = alpha * lambda_ic_u + ((1 - alpha) * (total_norm / (gnorms_u['ic'] + 1e-8)))
                lambda_res_u = alpha * lambda_res_u + ((1 - alpha) * (total_norm / (gnorms_u['res'] + 1e-8)))
                lambda_data_u = alpha * lambda_data_u + ((1 - alpha) * (total_norm / (gnorms_u['data'] + 1e-8)))

                lambda_ic_v = alpha * lambda_ic_v + ((1 - alpha) * (total_norm / (gnorms_v['ic'] + 1e-8)))
                lambda_res_v = alpha * lambda_res_v + ((1 - alpha) * (total_norm / (gnorms_v['res'] + 1e-8)))
                lambda_data_v = alpha * lambda_data_v + ((1 - alpha) * (total_norm / (gnorms_v['data'] + 1e-8)))
