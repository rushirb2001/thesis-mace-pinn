...
def create_optimizer(lr=1e-3, decay_steps=10000, decay_rate=1.0):
    schedule = optax.exponential_decay(lr, decay_steps, decay_rate)
    return optax.adam(schedule)

def train_step(params, opt_state, data_dict, optimizer, ep1, ep2, b1, c1, b2, c2):
    loss_val, grads = jax.value_and_grad(loss_fn)(params, data_dict, ep1, ep2, b1, c1, b2, c2)
    grads_clipped = clip_gradients(grads, max_norm=10.0)
...
def train_pinn_2d(
    ...
    for w_idx in range(num_windows):
        ...
        for epoch in tqdm(range(epochs_per_window), desc="PINN training (2D)"):
            ...
            params, opt_state, loss_val = train_step(params, opt_state, data_dict, optimizer,
                                                     ep1, ep2, b1, c1, b2, c2)
