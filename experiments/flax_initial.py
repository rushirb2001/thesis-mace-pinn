...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        for dim in self.hidden_dims:
            x = nn.Dense(dim, kernel_init=jax.nn.initializers.glorot_uniform())(x)
            x = nn.swish(x)
        x = nn.Dense(self.output_dim, kernel_init=jax.nn.initializers.glorot_uniform())(x)
        return x

# Initialize MLP parameters for u and v
key = random.PRNGKey(0)
dummy_input = jnp.ones((1, 3))  # Example input shape (x, y, t)

# Split keys for u and v networks
key, key_u, key_v = random.split(key, 3)

# Initialize u network
u_mlp = MLP(hidden_dims=[128, 128, 128, 128], output_dim=1)
params_u = u_mlp.init(key_u, dummy_input)['params']

# Initialize v network
v_mlp = MLP(hidden_dims=[128, 128, 128, 128], output_dim=1)
params_v = v_mlp.init(key_v, dummy_input)['params']

params = {"u": params_u, "v": params_v}

def pinn_predict_2d(params, inputs):
    x, y, t = inputs[:,0], inputs[:,1], inputs[:,2]
    x_wrap = (x + 1.0) % 2.0 - 1.0
    y_wrap = (y + 1.0) % 2.0 - 1.0
    mlp_input = jnp.stack([x_wrap, y_wrap, t], axis=-1)

    # Use Flax MLP to compute u and v
    u_out = u_mlp.apply({'params': params["u"]}, mlp_input).squeeze()
    v_out = v_mlp.apply({'params': params["v"]}, mlp_input).squeeze()
...
def create_optimizers(lr_u=1e-2, lr_v=1e-2, decay_steps=1000, decay_rate=0.95):
    sched_u = optax.exponential_decay(lr_u, 1000, decay_rate)
    sched_v = optax.exponential_decay(lr_v, 10000, decay_rate)
...
def train_step(params, opt_state_u, opt_state_v, data_dict, optim_u, optim_v, ep1, ep2, b1, c1, b2, c2):
    ...
    grad_u_clipped = clip_gradients(grad_u, max_norm=10.0)
    grad_v_clipped = clip_gradients(grad_v, max_norm=100.0)
    
    # Update U network
    updates_u, opt_state_u = optim_u.update(grad_u_clipped, opt_state_u, params["u"])
    new_params_u = optax.apply_updates(params["u"], updates_u)
    
    # Update V network
    updates_v, opt_state_v = optim_v.update(grad_v_clipped, opt_state_v, params["v"])
    ...
    return new_params, opt_state_u, opt_state_v, total_loss
...
def train_pinn_2d(
    ...
    """
    global lambda_ic_u, lambda_res_u, lambda_data_u, lambda_ic_v, lambda_res_v, lambda_data_v, alpha, update_freq
    ...
    for w_idx in range(num_windows):
        ...
        for epoch in tqdm(range(epochs_per_window), desc="PINN training (2D)"):
            ...
            if epoch % 1000 == 0:
                ...
                cond_ntk = jnp.linalg.cond(jnp.array(compute_ntk(params, subset))) if subset.shape[0] > 0 else jnp.nan
                ...
                # Update weights using moving average (separate for U/V)
                if epoch != 0:
                    lambda_ic_u = alpha * lambda_ic_u + ((1 - alpha) * (total_norm_u / (gnorms_u['ic'] + 1e-8)))
                    lambda_res_u = alpha * lambda_res_u + ((1 - alpha) * (total_norm_u / (gnorms_u['res'] + 1e-8)))
                    lambda_data_u = alpha * lambda_data_u + ((1 - alpha) * (total_norm_u / (gnorms_u['data'] + 1e-8)))

                    lambda_ic_v = alpha * lambda_ic_v + ((1 - alpha) * (total_norm_v / (gnorms_v['ic'] + 1e-8)))
                    lambda_res_v = alpha * lambda_res_v + ((1 - alpha) * (total_norm_v / (gnorms_v['res'] + 1e-8)))
                    lambda_data_v = alpha * lambda_data_v + ((1 - alpha) * (total_norm_v / (gnorms_v['data'] + 1e-8)))
                ...
                print(
                    f"╭──────── Window {float(w_idx):2.0} | Epoch {float(epoch):5.0} ────────╮\n"
                    f"│   Loss: {loss_val:.4e}   |  NTK Cond: {cond_ntk:.2e}\n"
                    f"├──────────── Gradient Norms ────────────────│\n"
...
def visualize_window_end(
    ...
    # 5) Output directory
    out_dir = f"figures_opt_lat/window_{w_idx}/epoch_{epoch}"
