...
# ====================================================
def logging_metrics(lambda_w, metrics, grad_norms, loss_val, epoch, alpha, update_lambda = [True, True], scaling_lambda = [False, False, True], term_log = True):
    ...
    if update_lambda[0]:
        lambda_w['ic_u'] = alpha*lambda_w['ic_u'] + (1-alpha)*(total_norm_u/(grad_norms['ic_u'] + 1e-8))
        lambda_w['data_u'] = alpha*lambda_w['data_u'] + (1-alpha)*(total_norm_u/(grad_norms['data_u'] + 1e-8))
        lambda_w['res_u'] = alpha*lambda_w['res_u'] + (1-alpha)*(total_norm_u/(grad_norms['res_u'] + 1e-8))

    # Update weights for V network
    if update_lambda[1]:
        lambda_w['ic_v'] = alpha*lambda_w['ic_v'] + (1-alpha)*(total_norm_v/(grad_norms['ic_v'] + 1e-8))
        lambda_w['data_v'] = alpha*lambda_w['data_v'] + (1-alpha)*(total_norm_v/(grad_norms['data_v'] + 1e-8))
        lambda_w['res_v'] = alpha*lambda_w['res_v'] + (1-alpha)*(total_norm_v/(grad_norms['res_v'] + 1e-8))

    # Apply constraints
    if scaling_lambda[0]:
        lambda_w['ic_u'] = jnp.clip(lambda_w['ic_u'], 1e-2, 1e-1)
        lambda_w['ic_v'] = jnp.clip(lambda_w['ic_v'], 1e-2, 1e-1)
    
    if scaling_lambda[1]:
        lambda_w['data_u'] = jnp.clip(lambda_w['data_u'], 1e2, 1e4)
        lambda_w['data_v'] = jnp.clip(lambda_w['data_v'], 1e2, 1e4)

    if scaling_lambda[2]:
        lambda_w['res_u'] = jnp.clip(lambda_w['res_u'], 1e1, 1e3)
        lambda_w['res_v'] = jnp.clip(lambda_w['res_v'], 1e1, 1e3)
    ...
    if term_log:
        print(
                ...
                f"│   U-Loss: {loss_val['u']:.4e} │  Alpha Vl: {alpha:.2e} │\n"
...
def train(config, data):
    ...
    beta = 1.00005 # Moving Average Momentum
    gamma = 1.00003 # Moving Average Momentum
    ...
    with tqdm(total=config['epochs']) as pbar:
        for epoch in range(config['epochs']):
            ...
            loss_val['u'] = metrics['data_u'] + metrics['ic_u'] + metrics['res_u']
            loss_val['v'] = metrics['data_v'] + metrics['ic_v'] + metrics['res_v']

            # Adaptive weighting every 100 epochs
            if metrics['total'] < best_loss and epoch % config['log_freq'] == 0:

                grad_norms = compute_component_gradients(state, batch, colloc_points, constants)
                lambda_w = logging_metrics(lambda_w, metrics, grad_norms, loss_val, epoch, alpha, [True, True], [True, False, True], True)

                best_loss = metrics['total']
                best_loss_u = loss_val['u']
                best_loss_v = loss_val['v']

                visualize_solution(state, data, epoch, t_start + window_size - 1)
            
            # elif metrics['total'] > best_loss and epoch % config['log_freq'] == 0:

            #     grad_norms = compute_component_gradients(state, batch, colloc_points, constants)

            #     if loss_val['u'] < best_loss_u:
            #         lambda_w = logging_metrics(lambda_w, metrics, grad_norms, loss_val, epoch, alpha, [True, False], [True, True, True])
            #         logging.info("For Model u")
            #         best_loss_u = loss_val['u']

            #     elif loss_val['v'] < best_loss_v:
            #         lambda_w = logging_metrics(lambda_w, metrics, grad_norms, loss_val, epoch, alpha, [False, True], [True, True, True])
            #         best_loss_u = loss_val['v']
            #         logging.info("For Model v")
