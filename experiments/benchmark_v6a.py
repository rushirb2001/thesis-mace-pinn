    ...
    with tqdm(total=config['epochs'], ncols=75) as pbar:
        for epoch in range(config['epochs']):
            ...
            if epoch % config['log_freq'] == 0:
                ...
                print(
                        f"\n╭──────── Window {float(99):2.0} | Epoch {float(epoch):5.0} ────────╮\n"
                        f"│   U-Loss: {loss_val_u:.4e} │  L2E Unet: {a:.2e} │\n"
                        f"│   V-Loss: {loss_val_v:.4e} │  L2E Vnet: {b:.2e} │\n"
                        f"├──────────── UV-Error-Norms ────────────────│\n"
                        f"│       Component      │  U-Net   │  V-Net   │\n"
                        f"│ {sep}Initial Condition  │ {grad_fmt(metrics['ic_u'])} │ {grad_fmt(metrics['ic_v'])} │\n"
                        f"│ {sep}Residual           │ {grad_fmt(metrics['res_u'])} │ {grad_fmt(metrics['res_v'])} │\n"
                        f"│ {sep}Data               │ {grad_fmt(metrics['data_u'])} │ {grad_fmt(metrics['data_v'])} │\n"
                        f"├──────────── Gradient Norms ────────────────│\n"
                        f"│       Component      │  U-Net   │  V-Net   │\n"
                        f"│ {sep}Initial Condition  │ {grad_fmt(grad_norms['ic_u'])} │ {grad_fmt(grad_norms['ic_v'])} │\n"
                        f"│ {sep}Residual           │ {grad_fmt(grad_norms['res_u'])} │ {grad_fmt(grad_norms['res_v'])} │\n"
                        f"│ {sep}Data               │ {grad_fmt(grad_norms['data_u'])} │ {grad_fmt(grad_norms['data_v'])} │\n"
                        f"├────────────── Loss Weights ────────────────│\n"
                        f"│       Component      │  U-Net   │  V-Net   │\n"
...
def visualize_solution(state, data, epoch, t_idx):
    ...
    print(f"[VIZ]==[Visualisation Saved to figures/epoch_{epoch}/solution_{t_idx}.png]==[VIZ]")
