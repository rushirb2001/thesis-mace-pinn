    ...
    def __call__(self, x):
        ...
        for i, dim in enumerate(self.hidden_dims[1:]):
            x = nn.Dense(dim, 
                        ...
                        kernel_init=nn.initializers.glorot_normal())(x) # Add Initial Coupling layer for Experiment to test if that increases the Residual performance.
            x = nn.tanh(x) # Experiment with tanh and no BatchNorm variation, as well as custom activation.
        # x = x + decoup
        x = nn.Dense(1, 
...
class GrayScottPINN(nn.Module):
    hidden_dims: list = struct.field(default_factory=lambda: [256, 256, 256, 128])
    ...
    def __call__(self, x):
        ...
        x_wrap = jnp.concatenate([
            (x[..., 0:1] + 1) % 2 - 1,  # x periodic in [-1, 1]
            (x[..., 1:2] + 1) % 2 - 1,  # y periodic in [-1, 1]
            x[..., 2:3] / 2    # t normalized to [0, 1]
        ], axis=-1)
        ...
        x_coup = ff
        # x_coup = nn.Dense(self.hidden_dims[0],
        #             name=f'head_dense_0',
        #             kernel_init=nn.initializers.glorot_normal())(x_coup)
        # x_coup = nn.tanh(x_coup)

        # Create submodules for u and v predictions
        u = MLP(self.hidden_dims[1:], name='u')(x_coup)
        v = MLP(self.hidden_dims[1:], name='v')(x_coup)
...
def compute_loss(state, params, batch, colloc_points, constants):
    ...
    targets_ic = batch['targets'][:40000]
    
    # IC loss 
    loss_ic = jnp.mean((u_pred[:40000] - targets_ic[:, 0])**2) + jnp.mean((v_pred[:40000] - targets_ic[:, 1])**2)
...
def compute_component_gradients(state, batch, colloc_points, constants):
    inputs_ic = batch['inputs'][:40000]
    targets_ic = batch['targets'][:40000]

    # Define individual loss functions
    def ic_loss_u(params):
        u, _ = state.apply_fn(params, inputs_ic)
        return jnp.mean((u - targets_ic[:, 0])**2)

    def ic_loss_v(params):
        _, v = state.apply_fn(params, inputs_ic)
        return jnp.mean((v - targets_ic[:, 1])**2)

    def res_loss_u(params):
        ru, _ = compute_residuals(state, params, colloc_points, constants)
        return jnp.mean(ru**2)
    ...
    # Compute gradients
    grad_ic_u = grad(ic_loss_u)(state.params)
    grad_ic_v = grad(ic_loss_v)(state.params)
    grad_res_u = grad(res_loss_u)(state.params)
    grad_res_v = grad(res_loss_v)(state.params)

    # Compute gradient norms per network
...
def train(config, data):
    ...
    with tqdm(total=config['epochs'], ncols=75) as pbar:
        for epoch in range(config['epochs']):
            ...
            if epoch % config['window_update_freq'] == 0 and window_size < config['max_window_size']:
                window_size += config['window_step']
                window_size = min(window_size, config['max_window_size'])

            window_end = t_start + window_size

            # Initial Condition
            inputs, targets = gather_window_data(data, t_start, t_start + 2)
            batch = {
                'inputs': inputs,
                'targets': targets
            }

            # Collocation points for current window timeframe
            ...
            if epoch % config['log_freq'] == 0:
                ...
                g_ic = (grad_norms['ic_u'] + grad_norms['ic_v']) / 2
                
                lambda_ic = alpha * lambda_ic + (1 - alpha) * (1 / (g_ic + 1e-8))
                lambda_res_u = alpha * lambda_res_u + (1 - alpha) * (1 / (grad_norms['res_u'] + 1e-8))
                lambda_res_v = alpha * lambda_res_v + (1 - alpha) * (1 / (grad_norms['res_v'] + 1e-8))
                
                total_weight = lambda_ic + lambda_res_u + lambda_res_v
                lambda_ic /= total_weight
                lambda_res_u /= total_weight
                lambda_res_v /= total_weight
                
                a, b = visualize_solution(state, data, epoch, t_start + window_size - 1)
                loss_val_u = metrics['res_u'] + metrics['ic']
                loss_val_v = metrics['res_v'] + metrics['ic']
                ...
                print(
                    f"\n╭───────────────── Epoch {epoch:5d} ───────────────╮╭─────────────── Window {window_size-1:2d} ───────────────╮\n"
                    ...
                    f"│ {sep}Initial Condition   │ {grad_fmt(metrics['ic'])} │ {grad_fmt(metrics['ic'])} ││ {grad_fmt(grad_norms['ic_u'])} │ {grad_fmt(grad_norms['ic_v'])} ││ {weight_fmt(lambda_ic)} │ {weight_fmt(lambda_ic)} │\n"
...
def create_gif(filenames, output_gif="flax_solution_comparison.gif", fps=60):
    ...
    with imageio.get_writer(output_gif, mode='I', fps=fps) as writer:
        for filename in tqdm(filenames, desc="Creating GIF", ncols=75):
...
if __name__ == "__main__":
    ...
    checkpoints.save_checkpoint(ckpt_dir='/home/rbhavsa4/checkpoints', target=trained_state, step=trained_state.step, overwrite=True)