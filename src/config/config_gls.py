...
if __name__ == "__main__":
    ...
    for i in tqdm(range(3,5)):
        variation = i
        run = wandb.init(project="ginzburg-landau-pinn-thesis-finale", config=config, reinit=True)
        data = load_grey_scott_mat(f"ginzburg_landau_square_{variation}.mat")
        trained_state = train(config, data)
        ckpt_dir = f"/home/rbhavsa4/final_checkpoints/ginzburg_landau/square_{variation}" 
