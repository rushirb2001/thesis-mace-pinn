...
class FourierFeatureLayer(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        B = B_init(key = jax.random.key(42), shape=(self.num_features // 2, self.input_dim))
...
class GrayScottPINNGated(nn.Module):
    ...
    def __call__(self, x):
        ...
        for i, dim in enumerate(self.hidden_dims):
            # U-subnetwork
            u = nn.Dense(dim, name=f'u/dense_{i}')(u)
            u = nn.tanh(u)
            
            # V-subnetwork
            v = nn.Dense(dim, name=f'v/dense_{i}')(v)
            v = nn.tanh(v)

            # Cross-gating
            u = CrossGate(name=f'u/gate_{i}')(u, v)
            v = CrossGate(name=f'v/gate_{i}')(v, u)

        # Final outputs
        u_out = nn.Dense(1, name='u/output')(u).squeeze()
        v_out = nn.Dense(1, name='v/output')(v).squeeze()
...
def create_train_state(rng, config, data):
    model = GrayScottPINNGated(  # Changed to use gated model
...
def compute_component_gradients(state, batch, colloc_points, constants):
    """Compute gradients for each loss component using submodule structure"""
    def network_grad_norm(grads, submodule):
        leaves = jax.tree_util.tree_leaves(grads['params'].get(submodule, {}))
        return jnp.sqrt(sum(jnp.sum(g**2) for g in leaves)) if leaves else 0.0
    ...
    return {
        'ic_u': network_grad_norm(grad_ic, 'u'),
        'ic_v': network_grad_norm(grad_ic, 'v'),
        'data_u': network_grad_norm(grad_data, 'u'),
        'data_v': network_grad_norm(grad_data, 'v'),
        'res_u': network_grad_norm(grad_res, 'u'),
        'res_v': network_grad_norm(grad_res, 'v')
    }
...
if __name__ == "__main__":
    ...
    data = load_grey_scott_mat(f"/Users/rushirbhavsar/Downloads/grey_scott_variation{variation}.mat")
    trained_state = train(config, data)
    ckpt_dir = f'/home/rbhavsa4/checkpoints_variation_{variation}'
