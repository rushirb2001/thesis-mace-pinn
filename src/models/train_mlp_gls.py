...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        for i, dim in enumerate(self.hidden_dims):
            ...
            x = nn.tanh(x)
        x = nn.Dense(2, name=f'{self.name}_output', kernel_init=nn.initializers.glorot_normal())(x)
        x = nn.tanh(x)
        return x
...
class GrayScottPINN(nn.Module):
    ...
    def __call__(self, x):
        ...
        uv = MLP(self.hidden_dims, name='uv')(ff)
        u, v = uv[..., 0], uv[..., 1]
...
def create_train_state(rng, config, data):
    ...
    tx = optax.chain(
        ...
        optax.lamb(optax.cosine_decay_schedule(
        config['lr'], 
        config['epochs'],
        alpha = 1e-04
        ))
...
def compute_loss(state, params, batch, colloc_points, constants):
    ...
    loss_ic = jnp.mean((u_pred[:n_ic] - targets_ic[:, 0])**2) + jnp.mean((v_pred[:n_ic] - targets_ic[:, 1])**2)
    
    # Data loss (second half of batch)
    loss_data = jnp.mean((u_pred[n_ic:] - targets_data[:, 0])**2) +  jnp.mean((v_pred[n_ic:] - targets_data[:, 1])**2)
    ...
    total_loss = (
        ...
        constants['lambda_data'] * loss_data +
        constants['lambda_res'] * loss_res
    )
...
def compute_component_gradients(state, batch, colloc_points, constants):
    ...
    return {
        'ic': network_norm(grad_ic, 'uv'),
        'data': network_norm(grad_data, 'uv'),
        'res': network_norm(grad_res, 'uv'),
    }
    ...
    with tqdm(total=config['epochs'], ncols=75) as pbar:
        for epoch in range(config['epochs']):
            ...
            if epoch % config['log_freq'] == 0:
                ...
                total_norm = grad_norms['ic'] + grad_norms['data'] + grad_norms['res']

                if metrics['total'] > best_loss:
                    # Update weights for U network
                    lambda_ic = alpha*lambda_ic + (1-alpha)*(total_norm/(grad_norms['ic'] + 1e-8))
                    lambda_data = alpha*lambda_data + (1-alpha)*(total_norm/(grad_norms['data'] + 1e-8))
                    lambda_res = alpha*lambda_res + (1-alpha)*(total_norm/(grad_norms['res'] + 1e-8))

                    # Normalise Lambda
                    total_lambda = lambda_data + lambda_ic + lambda_res
                    lambda_data = lambda_data / total_lambda
                    lambda_ic = lambda_ic / total_lambda
                    lambda_res = lambda_res / total_lambda

                loss_val = metrics['data'] + metrics['ic'] + metrics['res']
                ...
                #         f"│   U-Loss: {loss_val:.4e}  │ V-Loss: {loss_val:.4e}  ││  L2E Unet: {a:.2e}  │ L2E Vnet: {b:.2e}  │\n"
                #         f"╰─────────────────────────────────────────────╯╰────────────────────────────────────────────╯\n"
                #         f"╭────── Component ──────╭── Error ─╮╭── Grad ──╮╭─ Loss-W ─╮\n"
                #         f"│                       │   UV-Net ││  UV-Net  ││  UV-Net  │\n"
                #         f"│                       │   UV-Net ││  UV-Net  ││  UV-Net  │"
                #         f"│ │ Initial Condition   │ 2.34e-04 ││ 1.58e-02 ││ 8.52e-01 │"
                #         f"│ {sep}Initial Condition   │ {grad_fmt(metrics['ic'])} │ {grad_fmt(metrics['ic'])} ││ {grad_fmt(grad_norms['ic'])} │ {grad_fmt(grad_norms['ic'])} ││ {weight_fmt(lambda_ic)} │ {weight_fmt(lambda_ic)} │\n"
                #         f"│ {sep}Model PDE Residual  │ {grad_fmt(metrics['res'])} │ {grad_fmt(metrics['res'])} ││ {grad_fmt(grad_norms['res'])} │ {grad_fmt(grad_norms['res'])} ││ {weight_fmt(lambda_res)} │ {weight_fmt(lambda_res)} │\n"
                #         f"│ {sep}Reference Data Loss │ {grad_fmt(metrics['data'])} │ {grad_fmt(metrics['data'])} ││ {grad_fmt(grad_norms['data'])} │ {grad_fmt(grad_norms['data'])} ││ {weight_fmt(lambda_data)} │ {weight_fmt(lambda_data)} │\n"
                #         f"╰───────────────────────╰─────────────────────╯╰─────────────────────╯╰─────────────────────╯"
                #     )
                ...
                wandb.log({
                    ...
                    "ic_loss": metrics['ic'],
                    "data_loss": metrics['data'],
                    "res_loss": metrics['res'],
                    "l2_u": l2_u, "l2_v": l2_u,
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(0,5)):
        variation = i
        run = wandb.init(project="ginzburg-landau-pinn-thesis-finale-mlp", config=config, reinit=True)
        data = load_grey_scott_mat(f"ginzburg_landau_square_{variation}.mat")
        trained_state = train(config, data)
        ckpt_dir = f"/home/rbhavsa4/final_checkpoints/ginzburg_landau/mlp_square_{variation}" 
        abs_ckpt_dir = os.path.abspath(ckpt_dir)
        os.makedirs(abs_ckpt_dir, exist_ok=True)
        checkpoints.save_checkpoint(ckpt_dir=abs_ckpt_dir, target=trained_state, step=trained_state.step)
        # frames = generate_gif_frames_flax(
        #     trained_state, 
        #     data
        # )
        # create_gif(frames, "ginzburg_landau_solution_comparison.gif")
        run.finish()