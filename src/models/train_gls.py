...
class CrossGate(nn.Module):
    @nn.compact
    def __call__(self, inputs, gate_inputs):
        # inputs: (batch, features) from the current subnetwork
        # gate_inputs: (batch, features) from the other subnetwork
        gate = nn.Dense(inputs.shape[-1], 
                       kernel_init=nn.initializers.zeros,
                       name='gate_dense')(gate_inputs)
        gate = nn.sigmoid(gate)
        return inputs * gate  # Element-wise multiplication

class GrayScottPINNGated(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        # Input wrapping and Fourier features (existing code)
        ...
        # Initialize u and v features
        u = ff
        v = ff

        # Process through hidden layers with cross-gating
        for i, dim in enumerate(self.hidden_dims):
            # Dense layers
            u = nn.Dense(dim, name=f'u_dense_{i}')(u)
            u = nn.tanh(u)
            v = nn.Dense(dim, name=f'v_dense_{i}')(v)
            v = nn.tanh(v)

            # Cross-gating
            u = CrossGate(name=f'u_gate_{i}')(u, v)
            v = CrossGate(name=f'v_gate_{i}')(v, u)

        # Final output layers
        u_out = nn.Dense(1, name='u_output')(u)
        u_out = nn.tanh(u_out).squeeze()
        v_out = nn.Dense(1, name='v_output')(v)
        v_out = nn.tanh(v_out).squeeze()
...
class GrayScottPINN(nn.Module):
    hidden_dims: list = struct.field(default_factory=lambda: [256, 256, 128])
    fourier_features: int = struct.field(default=64)
    fourier_scale: float = struct.field(default=3.0)
...
def compute_residuals(state, params, colloc_points, constants):
    ...
    def residual_fn(x):
        ...
        residual_u = (u_grad[2] 
                      - (constants['eps'] * laplacian_u) 
                      - (constants['k'] * (u - u * (u**2 + v**2) + 1.5 * v * (u**2 + v**2))))
        
        residual_v = (v_grad[2] 
                      - (constants['eps'] * laplacian_v) 
                      - (constants['k'] * (v - v * (u**2 + v**2) - 1.5 * u * (u**2 + v**2))))
...
def train(config, data):
    ...
    with tqdm(total=config['epochs'], ncols=75) as pbar:
        for epoch in range(config['epochs']):
            ...
            if epoch % config['log_freq'] == 0:
                ...
                if metrics['total'] > best_loss:
                    ...
                    lambda_ic_u = alpha*lambda_ic_u + (1-alpha)*(total_norm_u/(grad_norms['ic_u'] + 1e-8))
                    lambda_data_u = alpha*lambda_data_u + (1-alpha)*(total_norm_u/(grad_norms['data_u'] + 1e-8))
                    lambda_res_u = alpha*lambda_res_u + (1-alpha)*(total_norm_u/(grad_norms['res_u'] + 1e-8))

                    # Update weights for V network
                    lambda_ic_v = alpha*lambda_ic_v + (1-alpha)*(total_norm_v/(grad_norms['ic_v'] + 1e-8))
                    lambda_data_v = alpha*lambda_data_v + (1-alpha)*(total_norm_v/(grad_norms['data_v'] + 1e-8))
                    lambda_res_v = alpha*lambda_res_v + (1-alpha)*(total_norm_v/(grad_norms['res_v'] + 1e-8))

                    # Apply constraints
                    # lambda_ic_u = jnp.clip(lambda_ic_u, 1e-3, 1e-2)
                    # lambda_data_u = jnp.clip(lambda_data_u, 1e-3, 1e-2)
                    # lambda_res_u = jnp.clip(lambda_res_u, 1e1, 1e3)

                    # lambda_ic_v = jnp.clip(lambda_ic_v, 1e-3, 1e-2)
                    # lambda_data_v = jnp.clip(lambda_data_v, 1e-3, 1e-2)
                    # lambda_res_v = jnp.clip(lambda_res_v, 1e1, 1e3)

                    # Normalise Lambda
                    total_lambda = lambda_data_u + lambda_data_v + lambda_ic_u + lambda_ic_v + lambda_res_u + lambda_res_v
                    lambda_data_u = lambda_data_u / total_lambda
                    lambda_data_v = lambda_data_v / total_lambda
                    lambda_ic_u = lambda_ic_u / total_lambda
                    lambda_ic_v = lambda_ic_v / total_lambda
                    lambda_res_u = lambda_res_u / total_lambda
                    lambda_res_v = lambda_res_v / total_lambda
                ...
                a, b = visualize_solution(state, data, epoch, t_start + window_size - 1)

                print(
                        f"\n╭───────────────── Epoch {float(epoch):5.0} ───────────────╮╭─────────────── Window {float(99):2.0} ───────────────╮\n"
                        f"│   U-Loss: {loss_val_u:.4e}  │ V-Loss: {loss_val_v:.4e}  ││  L2E Unet: {a:.2e}  │ L2E Vnet: {b:.2e}  │\n"
                        f"╰─────────────────────────────────────────────╯╰────────────────────────────────────────────╯\n"
                        f"╭────── Component ──────╭── UV-Error-Norms ───╮╭── Gradient Norms ───╮╭─── Loss Weights ────╮\n"
                        f"│                       │  U-Net   │  V-Net   ││  U-Net   │  V-Net   ││  U-Net   │  V-Net   │\n"
                        f"│ {sep}Initial Condition   │ {grad_fmt(metrics['ic_u'])} │ {grad_fmt(metrics['ic_v'])} ││ {grad_fmt(grad_norms['ic_u'])} │ {grad_fmt(grad_norms['ic_v'])} ││ {weight_fmt(lambda_ic_u)} │ {weight_fmt(lambda_ic_v)} │\n"
                        f"│ {sep}Model PDE Residual  │ {grad_fmt(metrics['res_u'])} │ {grad_fmt(metrics['res_v'])} ││ {grad_fmt(grad_norms['res_u'])} │ {grad_fmt(grad_norms['res_v'])} ││ {weight_fmt(lambda_res_u)} │ {weight_fmt(lambda_res_v)} │\n"
                        f"│ {sep}Reference Data Loss │ {grad_fmt(metrics['data_u'])} │ {grad_fmt(metrics['data_v'])} ││ {grad_fmt(grad_norms['data_u'])} │ {grad_fmt(grad_norms['data_v'])} ││ {weight_fmt(lambda_data_u)} │ {weight_fmt(lambda_data_v)} │\n"
                        f"╰───────────────────────╰─────────────────────╯╰─────────────────────╯╰─────────────────────╯"
                    )
...
def generate_gif_frames_flax(state, usol, vsol, t_data, x_data, y_data, output_dir="flax_gif_frames"):
    """
    Generate animation frames from model predictions at each time step.
    
    For each time value in t_data, the function generates a plot showing:
      - The reference solution (usol, vsol)
      - The model prediction
      - The absolute error between reference and prediction
    
    The frames are saved as PNG files in the specified output directory.
    """
    os.makedirs(output_dir, exist_ok=True)
    filenames = []
    Nx, Ny = 200, 200  # Grid resolution; adjust if necessary
    
    for t_idx in tqdm(range(len(t_data)), desc="Generating frames", ncols=150):
        U_ref = usol[t_idx]
        V_ref = vsol[t_idx]
        t_val = t_data[t_idx]

        # Create a spatial grid and stack inputs with the current time value
        X, Y = np.meshgrid(x_data, y_data, indexing='ij')
        inputs = jnp.stack([
            X.flatten(),
            Y.flatten(),
            jnp.full(X.flatten().shape, t_val)
        ], axis=-1)

        # Model prediction using the restored state
        U_pred, V_pred = state.apply_fn(state.params, inputs)
        U_pred = np.array(U_pred).reshape(Nx, Ny)
        V_pred = np.array(V_pred).reshape(Nx, Ny)
        ...
        # Create a figure with subplots for reference, prediction, and error maps
        fig, axes = plt.subplots(2, 3, figsize=(18, 12))
...
def generate_gif_frames_flax(state, data_dict, output_dir="flax_gif_frames"):
    ...
    for t_idx in tqdm(range(len(t_data)), desc="Generating frames", ncols=75):
...
if __name__ == "__main__":
    ...
    data = load_grey_scott_mat(f"ginzburg_landau_square.mat")
    trained_state = train(config, data)
    ckpt_dir = f'/home/rbhavsa4/checkpoints_glsv{variation}'
    checkpoints.save_checkpoint(ckpt_dir, target=trained_state, step=trained_state.step)
